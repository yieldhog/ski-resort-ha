...
- **Avalanche danger (US)** — opt-in; adds an *Avalanche danger* sensor with the
  avalanche.org rating for the resort's forecast zone (level, zone, travel
  advice, expiry, and forecast link as attributes). Keyless. **US only** —
  avalanche.org covers US avalanche centers, not Canada (Avalanche Canada) or
  Europe; resorts outside a US forecast zone won't report.
...
| Avalanche danger (rating + zone/advice/expiry) | sensor | avalanche.org (optional, US) |
